...
pub fn get(img: &Path, light: Brightness) -> Vec<Color> {
    ...
    eprintln!("colors len: {}", colors.len());
    adjust(&colors, light)
}

fn find_color(line: &str) -> Color {
    let start = line.find('#').unwrap();
    Color::from_hex(&line[start + 1 .. start + 7])
}

fn imagemagick(color_count: i32, img: &Path, magic_command: &str) -> Vec<Color> {
    let flags = [img.to_str().unwrap(), "-resize", "%25", "-colors", &color_count.to_string(), "-unique-colors", "txt:-"];
    let output = Command::new(magic_command)
                            .args(&flags)
                            .output()
                            .expect("failed to execute imagemagick");
    let colors = output.stdout;
    let colors: Vec<Color> = String::from_utf8(colors).expect("Failed to parse colors").lines().skip(1).map(|line| find_color(line)).collect();
    colors
}
...
fn adjust(colors: &[Color], light: Brightness) -> Vec<Color> {
    ...
    eprintln!("raw_colors len: {}", raw_colors.len());
    match light {
        Brightness::Light => {
